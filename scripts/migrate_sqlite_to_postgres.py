...
def migrate(source: Path) -> dict[str, tuple[int, int]]:
    ...
    try:
        ...
        with get_database_pool().connection() as destination, destination.transaction(), destination.cursor() as cursor:
            _execute_many(
                cursor,
                """
        ...
        with get_database_pool().connection() as destination, destination.cursor() as cursor:
            destination_counts = {}
            for table in TABLES:
